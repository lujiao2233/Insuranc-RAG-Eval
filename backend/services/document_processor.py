...
class DocumentProcessor:
    ...
    def process_pdf(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.info(f"PyMuPDF不可用或处理失败，回退到PyPDF2: {str(e)}")
    ...
    def process_docx(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        使用python-docx库解析Word文档，提取文本内容和表格数据。
        """
        try:
            if not os.path.exists(file_path):
                logger.warning(f"DOCX文件不存在: {file_path}")
                return None
            
            doc = DocxDocument(file_path)
            
            paragraph_count = len(doc.paragraphs)
            page_count = max(1, paragraph_count // 20)
            ...
            if tables_data:
                for index, table_rows in enumerate(tables_data, start=1):
                    text_content += f"\n表格{index}:\n"
