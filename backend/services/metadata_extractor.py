...
3. **列表项不是独立章节——以下情况必须与上文合并为同一节点，不得单独拆分**：
   - 以顿号（、）或分号（；）结尾的短语（如"投资研究、" "公司董事会；"），属于同一句话的并列成分。
   - 多个此类短语连续出现构成并列列表时，整个列表及其后紧跟的说明文字应合并为一个节点。
   - 标题后的冒号引导语（如"主要涉及的主体或部门："），属于该标题内容的组成部分，不是独立章节。
   - 判断方法：如果一段文字去掉顿号/分号后只是名词短语、而非完整的命题或规则，则它是列表项而非章节。
4. **公文公告与管理制度特别规则——短条款聚合**：
   - 当文档类型为 [公文公告] 或 [管理制度] 时，如果同一章节下存在多个连续的短条款（每条正文不足150字），应将它们按语义关联聚合为同一个二级节点，而不是逐条拆分为独立节点。
   - 聚合原则：
     a. 仅聚合同一父章节下的连续短条款，不可跨章节聚合。
     b. 聚合后的节点 title 使用首条编号范围表示（如"第三条至第五条"），anchor_text 使用首条的完整原文标题，以便精确定位到原文起始位置。
     c. 如果短条款之间存在明显语义断裂（如从"总则"跳到"罚则"），应在断裂处拆分为不同节点，不要强行聚合。
     d. 单条正文超过150字的条款不参与聚合，保持独立节点。
   - 示例：
     原始：第三条（20字）→ 第四条（30字）→ 第五条（50字）
     聚合后：一个节点，title="第三条至第五条"，anchor_text="第三条"，summary 概括三条的核心内容。
     原始：第六条（200字）→ 第七条（40字）
     结果：第六条独立节点 + 第七条可向下与第八条聚合。
5. **公文公告粗粒度提纲规则**：
   - 当文档类型为 [公文公告] 时，提纲应尽量粗粒度，确保每个节点的正文内容不少于200字（以提供足够的问题生成上下文）。
   - 短公文（全文不足800字）最多生成2-3个节点，不要按公文格式要素（文号、标题、主送、正文、落款）逐项拆分。推荐结构：一个"文件头"节点（文号+标题+主送单位），一个"正文"节点（全文核心内容+落款+日期）。
   - 中等长度公文（800-2000字）按语义段落划分，每段不少于200字。
   - 长公文（2000字以上）可正常按章节细分，但仍需满足每节点不少于200字。
6. 每个提纲项包含：
   - id: 唯一标识
   - level: 层级 (1, 2, 3)
   - title: 标题完整内容
